#!/usr/bin/env python3
"""
MM (Multi-Modal) Loihi 硬件效率分析工具 - 基于标准DSSA (W1=3x3, W2=3x3)

基于spikingresformer.py中的DSSA类（第44-107行）：
- W1: 3x3卷积
- W2: 3x3卷积  
- Wproj: 1x1卷积
- 使用Hadamard乘积实现门控
...
try:
    ...
    try:
        from models.MM_lava_dssa_shared import run_dssa_lava_shared as run_lava_native_simulation
...
class SpikeStatisticsCollector:
    ...
    def __init__(self, target_name):
        ...
        self.internal_stats = {
            'w1_output': [],
            'w2_output': [],
            'activation_attn_output': [],  # W1分支LIF
            'matmul1_output': [],  # attention计算
            'matmul2_output': [],  # attention应用
            'mul_output': [],  # Hadamard乘积（备用）
            'proj_output': []
        }
        self.hooks = []
    ...
    def register(self, model):
        for name, module in model.named_modules():
            if name == self.target_name:
                # 注册三个LIF的hook（DSSA有3个LIF：activation_in, activation_attn, activation_out）
                ...
                if hasattr(module, 'activation_attn'):
                    h = module.activation_attn.register_forward_hook(self._hook_activation_attn)
                    self.hooks.append(h)
                if hasattr(module, 'activation_out'):
                    h = module.activation_out.register_forward_hook(self._hook_activation_out)
                    self.hooks.append(h)
                ...
                if hasattr(module, 'W2'):
                    ...
                    self.hooks.append(h)
                # 标准DSSA使用 matmul1/matmul2 或 mul1（取决于flag_hardmard）
                if hasattr(module, 'matmul1'):
                    h = module.matmul1.register_forward_hook(self._hook_matmul1)
                    self.hooks.append(h)
                if hasattr(module, 'matmul2'):
                    h = module.matmul2.register_forward_hook(self._hook_matmul2)
                    self.hooks.append(h)
    ...
    def _hook_activation_attn(self, module, input, output):
        """收集W1分支LIF脉冲统计（activation_attn）"""
        with torch.no_grad():
            if isinstance(output, torch.Tensor) and len(output.shape) == 5:
                T, B, C, H, W = output.shape
                for t in range(T):
                    for b in range(B):
                        spike_count = (output[t, b] == 1).sum().item()
                        self.internal_stats['activation_attn_output'].append({
                            'time_step': t, 'batch': b, 'count': spike_count,
                            'total_elements': C * H * W,
                            'firing_rate': spike_count / (C * H * W)
                        })

    def _hook_activation_out(self, module, input, output):
        """收集DSSA输出脉冲统计（activation_out）"""
        with torch.no_grad():
            if isinstance(output, torch.Tensor) and len(output.shape) == 5:
                T, B, C, H, W = output.shape
                for t in range(T):
    ...
    def _hook_w1(self, module, input, output):
        self._collect_internal_stats('w1_output', output)

    def _hook_w2(self, module, input, output):
        self._collect_internal_stats('w2_output', output)

    def _hook_matmul1(self, module, input, output):
        """收集matmul1输出统计（attention计算）"""
        self._collect_internal_stats('matmul1_output', output)
    
    def _hook_matmul2(self, module, input, output):
        """收集matmul2输出统计（attention应用）"""
        self._collect_internal_stats('matmul2_output', output)

    def _hook_proj(self, module, input, output):
        self._collect_internal_stats('proj_output', output)

    def _collect_internal_stats(self, key, output):
        """收集中间层统计"""
        with torch.no_grad():
            if isinstance(output, torch.Tensor) and len(output.shape) == 5:
                T, B, C, H, W = output.shape
                for t in range(T):
                    for b in range(B):
                        ...
                        self.internal_stats[key].append({
                            'time_step': t,
                            'batch': b,
                            'count': spike_count,
                            'firing_rate': spike_count / (C * H * W) if C * H * W > 0 else 0
                        })
    ...
    def get_summary(self):
        ...
        output_firing_rates = [s['firing_rate'] for s in self.output_spikes] if self.output_spikes else []
        ...
        summary = {
            ...
            'output_avg_firing_rate': np.mean(output_firing_rates) if output_firing_rates else 0,
            'output_std_firing_rate': np.std(output_firing_rates) if output_firing_rates else 0,
            'total_input_spikes': sum([s['count'] for s in self.input_spikes]),
            'total_output_spikes': sum([s['count'] for s in self.output_spikes]) if self.output_spikes else 0,
            'internal': {}
        }
...
class DSSAAnalyzer:
    ...
    def compute_sop_complexity(self, spike_stats, T, H, W):
        ...
        【标准DSSA架构 - 基于spikingresformer.py】
      ...
      transpose         │               │
       ...
       y1'' ────────────┴───────────────┼──→ matmul1(y1'', x')
        ...
        【SOP计算 - 参考main.py SOPMonitor】
        SOP = 输入到某层的脉冲数 × fanout
        
        关键层：
        - W1/W2: Conv3x3, fanout = 9×C
        - matmul1: 矩阵乘法 (y1^T @ x), SOP ≈ 0（脉冲生成阶段）
        - matmul2: 矩阵乘法 (y2 @ attn), SOP ≈ 0（脉冲应用阶段）
        ...
        # activation_attn的输出（W1分支LIF后）
        activation_attn_output = internal_stats.get('activation_attn_output', {}).get('total_spikes', 0)
        
        # activation_out的输出（W2分支LIF后）
        activation_out_output = spike_stats.get('total_output_spikes', 0)
        
        # matmul1和matmul2的输出统计
        matmul1_output = internal_stats.get('matmul1_output', {}).get('total_spikes', 0)
        matmul2_output = internal_stats.get('matmul2_output', {}).get('total_spikes', 0)
        
        # Wproj的输入：matmul2的输出（如果统计不到则使用activation_out）
        wproj_input = activation_out_output
        ...
        D = C
        # matmul1: y1^T @ x
        # 输入：y1(浮点)和x(脉冲)，D=C, N=H*W
        # 输出 attn 维度: [T,B,1,N,N]
        # 对于 x 中的每个脉冲，需要与 y1^T 中的一列（N个元素）相乘
        # SOP = x_spike_count × N
        matmul1_fanout = N
        sop_matmul1 = activation_in_output * matmul1_fanout
        
        # matmul2: y2 @ attn_spike
        # 输入：y2(浮点)和attn_spike(脉冲)，D=C, N=H*W
        # 输出 out 维度: [T,B,1,D,N]
        # 对于 attn_spike 中的每个脉冲，需要与 y2 中的一行（D个元素）相乘
        # SOP = attn_spike_count × D
        matmul2_fanout = D
        sop_matmul2 = activation_attn_output * matmul2_fanout 
        ...
        # 总SOP（所有有突触操作的层）
        # W1: Conv3x3, W2: Conv3x3, matmul1/matmul2: 矩阵乘法, Wproj: Conv1x1
        total_sops = sop_w1 + sop_w2 + sop_matmul1 + sop_matmul2 + sop_wproj
        ...
        macs_w1_ann = positions_per_frame * C * C * 3 * 3
        macs_w2_ann = positions_per_frame * C * C * 3 * 3
        macs_matmul1_ann = positions_per_frame * N * C  
        macs_matmul2_ann = positions_per_frame * D * C  
        macs_wproj_ann = positions_per_frame * C * C
        total_macs_ann = macs_w1_ann + macs_w2_ann + macs_matmul1_ann + macs_matmul2_ann + macs_wproj_ann
        ...
        return {
            'analysis_type': 'SNN实际SOP统计（基于DSSA架构）',
            'platform': 'Loihi-like Neuromorphic Hardware',
            'dssa_architecture': {
                'data_flow': 'x → [W1→matmul1, W2] → matmul2 → Wproj',
                'note': 'Standard DSSA with matmul-based attention',
            },
            'spike_flow': {
                ...
                'w1_output': w1_output,
                'w2_output': w2_output,
                'activation_attn_output': activation_attn_output,
                'activation_out_output': activation_out_output,
                'matmul1_output': matmul1_output,
                'matmul2_output': matmul2_output,
            ...
            'sop_breakdown': {
                ...
                'matmul1': {
                    'input': activation_attn_output,
                    'fanout': matmul1_fanout,
                    'sop': sop_matmul1,
                    'note': 'Matrix multiplication (y1^T @ x), spike generation',
                },
                'matmul2': {
                    'input': activation_out_output,
                    'fanout': matmul2_fanout,
                    'sop': sop_matmul2,
                    'note': 'Matrix multiplication (y2 @ attn), spike application',
                },
                'Wproj_1x1': {
                    'input': wproj_input,
                    'fanout': wproj_fanout,
    ...
    def compute_communication_complexity(self, T, H, W, mapping_strategy='token_wise'):
        ...
        """
        SSA核心：Mul（哈达玛积）的通信复杂度
        """
        # SSA中：Q_local 和 V_local 是独立分支，可能映射到不同核心
        # Token-wise下，W1分支和W2分支分别映射到不同的核心组
        # Mul需要Q从W1核心传输到V所在的W2核心 → NoC通信
        # 每个位置的Q(D维)需要跨核心传输到对应V位置
        # 总通信量 = T × N × D = O(ND)
        
        # 标准DSSA (MM): matmul通信
        # y1和y2都是局部的，matmul需要数据在核心间传输
        matmul_comm = T * N * D  # O(ND)
        
        return {
            # 通信复杂度
            'comm_matmul': matmul_comm,  # O(ND) - matmul计算的数据传输
            'comm_standard_attention': comm_standard_attention,  # O(N²D) - 标准attention对比
            
            # 加速比（相对于标准attention）
            'speedup_vs_standard': comm_standard_attention/matmul_comm if matmul_comm > 0 else float('inf'),
        }
...
def run_lava_simulation(features, T, dssa_module, spike_stats, weight_scale=1.0):
    ...
    print(f"\n{'='*60}")
    print("Lava Token-wise 仿真（完整DSSA模块）")
    print(f"{'='*60}")
    print("网络结构: Input → (W1→LIF1, W2→LIF2) → Mul → Wproj → Output")
...
def estimate_energy_with_routing(T, C, H, W, lava_stats=None, features=None):
    """
    ...
    # 获取LIF输出脉冲
    l1_spikes = int(lava_stats.get('l1_spikes', [0])[0]) 
    l2_spikes = int(lava_stats.get('l2_spikes', [0])[0])
    
    # 获取 monitor_data 中的脉冲统计（兼容 SSA 架构的 gated_out）
    monitor_data = lava_stats.get('monitor_data', {})
    mul_spikes = int(monitor_data.get('gated_out', np.array([0])).sum())  # SSA 门控输出，MM 为 0
    
    # attn_spikes 是 LIF1 的输出脉冲（即 l1_spikes）
    attn_spikes = l1_spikes  # MM 架构: LIF1 输出 = attn 脉冲 
    ...
    # DSSA数据流（MM 架构）：
    #   activation_in输出 → W1(3x3) → LIF1 → matmul1(y1^T @ x)
    #   activation_in输出 → W2(3x3) → LIF2 ─┘
    #   matmul1输出 → LIF_attn ──────────────→ matmul2(y2 @ attn)
    #   matmul2输出 → Wproj(1x1)
    # 
    # SOP计算：输入脉冲数 × fanout (每个输入连接的输出数)
    # - W1/W2: 3×3 卷积核 = 9 个连接 × C 输出通道 = 9C fanout
    # - Wproj: 1×1 卷积核 = 1 个连接 × C 输出通道 = 1C fanout
    # - matmul1: y1^T @ x，每个 x 脉冲与 N 个位置相乘
    # - matmul2: y2 @ attn，每个 attn 脉冲与 D 个通道相乘
    sop_w1 = estimated_input_spikes * 9 * C   # W1: 3x3 卷积
    sop_w2 = estimated_input_spikes * 9 * C   # W2: 3x3 卷积（相同输入）
    
    # matmul1: y1^T @ x, y1来自W1(浮点), x是脉冲
    # SOP = x_spikes × N (每个x脉冲与N个位置相乘)
    sop_matmul1 = estimated_input_spikes * N
    
    # matmul2: y2 @ attn，attn是LIF1输出脉冲, y2来自W2(浮点)
    # 矩阵乘法: [N,N] @ [N,D] = [N,D]
    # 每个attn脉冲(i,j)参与D个输出位置的计算(out[i,0:D])
    sop_matmul2 = l1_spikes * D  # 每个脉冲影响D个输出位置
    
    # Wproj: 1x1卷积，输入是LIF2输出
    wproj_input_spikes = l2_spikes
    sop_wproj = wproj_input_spikes * 1 * C

    
    synaptic_ops = sop_w1 + sop_w2 + sop_matmul1 + sop_matmul2 + sop_wproj
    
    synapse_energy = synaptic_ops * E_synapse_op
    
    # 2. 神经元更新能耗 (W1 + W2分支，区分active/inactive)
    # 
    # 标准DSSA (MM) 特性：W1和W2都是3x3，各有 N×C 个LIF神经元
    # - W1: N×C 个神经元（用于计算attention）
    # - W2: N×C 个神经元（用于加权输出）
    
    num_active_tokens = lava_stats.get('num_tokens', H * W)  # 活跃token数
    num_neurons_w1 = num_active_tokens * C  # W1分支: N×C个神经元
    num_neurons_w2 = num_active_tokens * C  # W2分支: N×C个
    ...
    neuron_updates = total_active_updates + total_inactive_updates
    ...
    # 
    # 标准DSSA (MM) 数据流中的NoC通信：
    # 4a. W1和W2分支的邻居通信（3x3卷积需要8个邻居）
    # 每个活跃token需要从8个邻居接收输入
    # 4 个正交方向：4 × 1 = 4 跳 4 个对角方向：4 × 2 = 8 跳 平均：(4 + 8) / 8 = 1.5 跳（本地邻居）
    w12_neighbor_hops = num_active_tokens *C* 8 * 1.5 * 2  # 每个活跃token的8个邻居
    w12_noc_energy = w12_neighbor_hops * E_noc_hop_avg
    
    # 4b. MM 的 matmul 通信 (All-to-All with Multicast Trees)
    # Token-wise 映射: N 个 tokens 映射到 2D mesh
    # 多播树模型: 一个脉冲从源广播到所有 N 个目标的平均链路使用数
    # 在 2D mesh 上使用多播树: O(log N) 每脉冲 (树的高度)
    import math
    hops_multicast = math.log2(N) if N > 1 else 0  # 多播树高度 ≈ log2(N)
    
    # matmul1: y1^T @ x
    # 计算语义: attn[i,j] = dot(y1[i], x[j]) 对所有 i,j
    # - y1[i]: 本地驻留在 Core i (Receiver-only，不传输)
    # - x[j]: 从 Core j 通过多播树广播到所有 Cores i
    # 通信成本: x 脉冲数 × 多播树跳数
    matmul1_hops = estimated_input_spikes * hops_multicast
    matmul1_noc_energy = matmul1_hops * E_noc_hop_avg
    
    # matmul2: attn @ y2
    # 计算语义: out[i] = sum_j attn[i,j] * y2[j]
    # - attn[i,j]: Core i 本地 (作为索引/权重)
    # - y2[j]: 从 Core j 通过多播树按需广播到需要它的 Cores i
    # 按需多播: 仅当 attn[i,j]=1 时 y2[j] 需要到达 Core i
    # 成本: 活跃的 y2[j] 数量 × 多播树跳数
    # 估算: l1_spikes / N 是平均每个 y2[j] 被多少个 Core 需要
    active_y2 = min(l1_spikes, N)  # 保守估计: 需要广播的 y2 源数量
    matmul2_hops = active_y2 * hops_multicast
    matmul2_noc_energy = matmul2_hops * E_noc_hop_avg
    
    # 总NoC能耗
    total_noc_energy = w12_noc_energy + matmul2_noc_energy + matmul1_noc_energy
    
    # within-tile能耗（本地计算）
    # MM 架构: LIF2 输出直接到 Wproj，都是本地操作
    within_tile_spikes = l2_spikes  # LIF2 → Wproj (同一 Core 内)
    within_tile_energy = within_tile_spikes * E_spike_within_tile
    
    print(f"   NoC通信统计:")
    print(f"     W1/W2邻居通信: {w12_neighbor_hops:,} hops = {w12_noc_energy*1e6:.3f} μJ")
    print(f"     matmul1(y1^T@x): {matmul1_hops:,.0f} hops = {matmul1_noc_energy*1e6:.3f} μJ (全局All-to-All)")
    print(f"     matmul2(v@attn): {matmul2_hops:,.0f} hops = {matmul2_noc_energy*1e6:.3f} μJ (全局All-to-All)")
    ...
    return {
        ...
        'energy_breakdown': {
            ...
            'noc_w12_neighbor_j': w12_noc_energy,
        ...
        'operation_counts': {
            ...
            'l1_spikes': l1_spikes,
            'l2_spikes': l2_spikes,
            'attn_spikes': attn_spikes,
            'wproj_input_spikes': wproj_input_spikes, 
            'noc_w12_neighbor_hops': w12_neighbor_hops,
            'noc_matmul1_hops': matmul1_hops,  
            'noc_matmul2_hops': matmul2_hops , 
            'within_tile_spikes': within_tile_spikes,
            # SOP 分解（用于能耗显示）
            'sop_w1': sop_w1,
            'sop_w2': sop_w2,
            'sop_matmul1': sop_matmul1,
            'sop_matmul2': sop_matmul2,
            'sop_wproj': sop_wproj,
        },
        ...
        'notes': [
            ...
            'MM (标准DSSA) 特性：W1=W2=3x3, Q→V需要跨核心传输',
            '内存访问和静态能耗未计算',
        ]
    }
...
def run_hardware_analysis(checkpoint_path, config_path, data_path, num_samples=10,
    ...
    for sample_idx in range(num_samples):
        ...
        print(f"  脉冲统计:")
        print(f"    输入发放率: {spike_stats['input_avg_firing_rate']*100:.4f}%")
        print(f"    输出发放率: {spike_stats['output_avg_firing_rate']*100:.4f}%")
        ...
        print(f"    DSSA(MM)通信量: O(ND) = {comm_complexity['comm_matmul']:,.0f}")
        # 格式化加速比显示（大数用K/M表示）
        speedup_vs_std = comm_complexity['speedup_vs_standard']
        if speedup_vs_std >= 1000000:
            speedup_str = f"{speedup_vs_std/1000000:.1f}M"
        elif speedup_vs_std >= 1000:
            speedup_str = f"{speedup_vs_std/1000:.1f}K"
        else:
            speedup_str = f"{speedup_vs_std:.1f}"
        print(f"    DSSA vs 标准Attention加速比: {speedup_str}x (N倍)")
        ...
        # 能耗估算（包含路由）
        energy_estimate = estimate_energy_with_routing(T, structure['dim'], H_feat, W_feat, lava_stats, features)
        ...
        if energy_estimate:
            ...
            # 显示详细的 SOP 能耗分解
            sop_total = energy_estimate['operation_counts']['synaptic_operations']
            ops = energy_estimate['operation_counts']
            sop_w1 = ops.get('sop_w1', sop_total * 0.1)
            sop_w2 = ops.get('sop_w2', sop_total * 0.1)
            sop_matmul1 = ops.get('sop_matmul1', 0)
            sop_matmul2 = ops.get('sop_matmul2', 0)
            
            sop_wproj = ops.get('sop_wproj', 0)
            if sop_matmul1 > 0:
                # MM 架构：显示 matmul 能耗
                print(f"    │  ├─ W1(3x3): {sop_w1 * 81e-6:.2f} μJ ({sop_w1/sop_total*100:.1f}%)")
                print(f"    │  ├─ W2(3x3): {sop_w2 * 81e-6:.2f} μJ ({sop_w2/sop_total*100:.1f}%)")
                print(f"    │  ├─ matmul1(y1^T@x): {sop_matmul1 * 81e-6:.2f} μJ ({sop_matmul1/sop_total*100:.1f}%) ⚠️")
                print(f"    │  ├─ matmul2(v@attn): {sop_matmul2 * 81e-6:.2f} μJ ({sop_matmul2/sop_total*100:.1f}%)")
                print(f"    │  └─ Wproj(1x1): {sop_wproj * 81e-6:.2f} μJ ({sop_wproj/sop_total*100:.1f}%)")
            else:
                # SSA 架构：简化显示
            ...
            print(f"    │  ├─ W1/W2邻居通信: {energy_estimate['operation_counts']['noc_w12_neighbor_hops']:,} hops")
            if sop_matmul1 > 0:
                # MM 架构
                print(f"    │  ├─ matmul1全局通信: {energy_estimate['operation_counts'].get('noc_matmul1_hops', 0):,.0f} hops")
                print(f"    │  ├─ matmul2全局通信: {energy_estimate['operation_counts'].get('noc_matmul2_hops', 0):,.0f} hops")
                wproj_input = energy_estimate['operation_counts'].get('wproj_input_spikes', 0)
                wproj_input_scalar = int(wproj_input) if hasattr(wproj_input, '__iter__') else wproj_input
                print(f"    │  └─ Wproj本地通信: {wproj_input_scalar * C:,} hops")
                total_noc_hops = (energy_estimate['operation_counts']['noc_w12_neighbor_hops'] + 
                                  energy_estimate['operation_counts'].get('noc_matmul1_hops', 0) +
                                  energy_estimate['operation_counts'].get('noc_matmul2_hops', 0))
            else:
                # SSA 架构
                print(f"    │  └─ Q→V传输: {energy_estimate['operation_counts']['noc_q_to_v_hops']:,} hops")
                total_noc_hops = energy_estimate['operation_counts']['noc_w12_neighbor_hops'] + energy_estimate['operation_counts']['noc_q_to_v_hops']
            ...
            print(f"    - 神经元更新: {energy_estimate['operation_counts']['neuron_updates']:,}")
            print(f"    - 总NoC跳数: {total_noc_hops:,.0f}")
