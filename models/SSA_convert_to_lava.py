...
try:
    ...
    try:
        from models.SSA_lava_dssa_shared import run_dssa_lava_shared as run_lava_native_simulation
...
class SpikeStatisticsCollector:
    ...
    def _hook_mul(self, module, input, output):
        self._collect_internal_stats('mul_output', output)
...
class DSSAAnalyzer:
    ...
    def compute_sop_complexity(self, spike_stats, T, H, W):
        ...
        # activation_out的输出（W2分支LIF后，也是DSSA最终输出）
        activation_out_output = spike_stats.get('total_output_spikes', 0)
        
        # mul1的输入：activation_out输出 × activation_attn输出
        # mul1的输出
        ...
        return {
            ...
            'dssa_architecture': {
                'data_flow': 'x → activation_in → [W1→activation_attn, W2→activation_out] → mul1 → Wproj',
                'note': 'Two-branch structure with element-wise multiplication',
            },
            'spike_flow': {
...
def estimate_energy_with_routing(T, C, H, W, lava_stats=None, features=None):
    ...
    print(f"    SOP: W1(3x3)={sop_w1:,}, W2(3x3)={sop_w2:,}, Wproj(1x1)={sop_wproj:,}, 总计={synaptic_ops:,}")
    synapse_energy = synaptic_ops * E_synapse_op
    
    # 2. 神经元更新能耗 (W1 + W2分支，区分active/inactive)
    # 
    # SSA特性：W1和W2都是局部计算，各有 N×C 个LIF神经元
    # - W1: N×C 个神经元（局部Q）
    # - W2: N×C 个神经元（局部V）
    
    num_active_tokens = lava_stats.get('num_tokens', H * W)  # 活跃token数
    num_neurons_w1 = N * C  # SSA: 局部Q有N×C个神经元
    ...
    # 4b. SSA Q→V通信
    # Q从W1核心传输到W2核心（同一位置，不同计算单元）
    # 平均跳数 ~1（同一位置附近的核心）
    q_to_v_hops = l1_spikes * 1  # 每个L1脉冲传输1hop到V核心
    q_to_v_energy = q_to_v_hops * E_noc_hop_avg
    
    # 总NoC能耗
    total_noc_energy = w12_noc_energy + q_to_v_energy
    
    # within-tile能耗（本地计算）
    within_tile_spikes = l2_spikes + mul_spikes  # V→Mul→Wproj本地
    ...
    print(f"     Q→V传输: {q_to_v_hops:,} hops = {q_to_v_energy*1e6:.3f} μJ")
...
def run_hardware_analysis(checkpoint_path, config_path, data_path, num_samples=10,
    ...
    for sample_idx in range(num_samples):
        ...
        if energy_estimate:
            ...
            print(f"    - 总NoC跳数: {energy_estimate['operation_counts']['noc_w12_neighbor_hops'] + energy_estimate['operation_counts']['noc_q_to_v_hops']:,}")
